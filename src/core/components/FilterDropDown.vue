...
const filteredItems = computed(() => {
  ...
  return props.items.filter((i) => i.name.includes(search.value.trim()))
})
  ...
  <Menu as="div" class="relative inline-block text-left bg-base-white">
    ...
    >
      <MenuItems
        ...
        class="absolute z-[999] bg-base-white py-2 left-0 mt-2 origin-top-left rounded-md bg-white shadow-lg focus:outline-none"
      >
        <div v-if="props.hasSearch" class="px-2 py-2">
          <input v-model="search" type="text" placeholder="Search" class="form-input pr-8" />
        </div>
            ...
            <template v-for="item in filteredItems" :key="item.id">
              <div v-if="!item.hidden" class="flex items-center space-x-4 px-4 py-2">
                <input
                  ...
                  class="form-checkbox"
                  @change="(event) => checkItem(event, item.id)"
                />
                <span>{{ item.name }}</span>
              </div>
            </template>
